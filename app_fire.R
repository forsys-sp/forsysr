...
fire_scn_inputs = expand.grid(DECILE = 9, FUTURE = 1:10)
...
fire_scn_inputs %>% pmap(function(...){
  ...
  run(config_file = 'config_TenYearPlan_WW_FS.R', write_tags='_Dyn2', dynamic_forsys = TRUE, fire_intersect_table = fires_i, ...)
})
...
process_outputs('config_TenYearPlan_WW_FS.R',
                ...
                fires_dat = fread(input_stand_fire_intersect) %>%
                  ...
                  filter(DECILE == 9, FUTURE %in% c(1:3)))
