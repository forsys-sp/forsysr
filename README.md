...
code. The repository is currently private, which means that you need to
create a personal access token in order to install the package in R. The
token can be generated by going to the following url
...
will download and install the forsysr package to your computer. (Note
that in order to use patchmax, you also need to install that packages as
well).
