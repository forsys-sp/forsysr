...
that in order to use the *Patchmax* package, you need to install that
package as well. *Patchmax* is a binary package and the correct version
should be downloaded based on the user’s operating system (currently
available for either Mac or Windows) from the *forsys* repository:
<https://github.com/forsys-sp/forsysr/releases/tag/Patchmax_0.1.1>. We
recommend updating other packages when prompted.
...
#Windows
remotes::install_deps("your_filepath_here/Patchmax_0.1.1.zip")
install.packages("your_filepath_here/Patchmax_0.1.1.zip", repos = NULL, type = "win.binary")
#Mac
remotes::install_deps("your_filepath_here/Patchmax_0.1.1.tgz")
install.packages("your_filepath_here/Patchmax_0.1.1.tgz", repos = NULL, type = "mac.binary")
```
...
We then filter the results to observe the outcome of the scenario where
...
*Patchmax*, which requires some additional arguments.
