...
#'
#' @export
attainment_data_filter <- function(results_data,
                                   priority,
                                   constraint_field,
                                   secondary_effects = c()) {
  ...
  # Lastly get the ETrt_*_PCP name for the constraint. Note it might come in
  # ok already, but this guarantees it
  ...
  # Check the length of weight_columns. A single priority run will have only
  # one, otherwise we need to negate the main priority and select only the run
  # where the other priorities had 0 weight
  ...
  # Sort on treatment rank JUST IN CASE it got out of order somewhere, then
  # append the x and y columns
  g <- results_data %>%
...
#'
#' @export
attainment_chart_by_target_treated <- function(results_data,
                                               priority,
                                               constraint_field,
                                               secondary_effects=c()) {
...
#'
#' @export
cumulative_attainment_chart <- function(results_data,
                                        priority,
                                        constraint_field,
                                        secondary_effects=c()) {
  g <- attainment_data_filter(results_data, priority, constraint_field, secondary_effects)
